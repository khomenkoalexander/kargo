 ...
 *
 *  0-15  : transferId (UUID MSB + LSB)
 * 16-23  : byteOffset (long)
 * 24-27  : seq        (int)
 * 28-31  : length     (int)
 * 32-35  : crc32      (int)
...
public class ChunkEnvelope {
    ...
    public static ChunkEnvelope decode(byte[] data) {
        if (data.length < HEADER_SIZE) {
            throw new IllegalArgumentException("Data too short for chunk header");
        }
        ByteBuffer buf = ByteBuffer.wrap(data);

        ChunkEnvelope e = new ChunkEnvelope();
        e.transferId = new UUID(buf.getLong(), buf.getLong());
